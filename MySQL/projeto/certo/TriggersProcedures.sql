...
BEGIN
	INSERT INTO log_produto(mensagem) 
...
-- cria pedido
delimiter //
create procedure createPedido(usuario int, endereco int, total decimal, ob varchar(255))
begin
...
-- atualizar preco hamburguer
DELIMITER //

CREATE TRIGGER tr_calc_preco_hamburguer
BEFORE INSERT ON tb_hamburguer
FOR EACH ROW
BEGIN
    DECLARE preco_pao DECIMAL(10,2);
    DECLARE preco_carne DECIMAL(10,2);
    DECLARE preco_salada DECIMAL(10,2);
    DECLARE preco_molho DECIMAL(10,2);
    DECLARE preco_queijo DECIMAL(10,2);
    DECLARE preco_total DECIMAL(10,2);

    SELECT valor INTO preco_pao FROM tb_pao WHERE cod_pao = NEW.cod_pao;

    IF NEW.cod_carne IS NOT NULL THEN
        SELECT valor INTO preco_carne FROM tb_carne WHERE cod_carne = NEW.cod_carne;
    ELSE
        SET preco_carne = 0;
    END IF;

    IF NEW.cod_salada IS NOT NULL THEN
        SELECT valor INTO preco_salada FROM tb_salada WHERE cod_salada = NEW.cod_salada;
    ELSE
        SET preco_salada = 0;
    END IF;

    IF NEW.cod_molho IS NOT NULL THEN
        SELECT valor INTO preco_molho FROM tb_molho WHERE cod_molho = NEW.cod_molho;
    ELSE
        SET preco_molho = 0;
    END IF;

    IF NEW.cod_queijo IS NOT NULL THEN
        SELECT valor INTO preco_queijo FROM tb_queijo WHERE cod_queijo = NEW.cod_queijo;
    ELSE
        SET preco_queijo = 0;
    END IF;

    SET preco_total = preco_pao + preco_carne + preco_salada + preco_molho + preco_queijo;

    SET NEW.valorTotal = preco_total;
END //
...
CREATE TRIGGER tr_update_pedido_preco
AFTER INSERT ON tb_pedido_hamburguer
FOR EACH ROW
BEGIN
    DECLARE preco_hamburguer DECIMAL(10,2);
    DECLARE preco_total_pedido DECIMAL(10,2);

    SELECT valorTotal INTO preco_hamburguer FROM tb_hamburguer WHERE cod_hamburguer = NEW.cod_hamburguer;

    SELECT valor_total INTO preco_total_pedido FROM tb_pedido WHERE cod_pedido = NEW.cod_pedido;

    UPDATE tb_pedido
    SET valor_total = preco_total_pedido + (preco_hamburguer * NEW.quantidade)
...
delimiter //
create procedure adicionalHamburguer(cod_hamburguer int, cod_adicional int, quantidade int, cod_pedido int)
begin
	insert into tb_hamburguer_adicional (cod_hamburguer, cod_adicional, quantidade) values (cod_hamburguer, cod_adicional, quantidade);
    set @preco = (select valor from tb_adicional where cod_adicional = cod_adicional limit 1);
...
delimiter //
create procedure produtoPedido(cod_pedido int, cod_produto int, quantidade int)
begin
insert into tb_pedido_produto(cod_pedido, cod_produto, quantidade) values (cod_pedido, cod_produto, quantidade);
set @preco = (select valor from tb_produto where cod_produto = cod_produto limit 1 );
update tb_pedido set valor_total = valor_total + @preco;
end;
// delimiter ;

delimiter //
create procedure adicionaisDoHamburguer(cod_hamburguer int)
BEGIN
SELECT
	p.cod_pedido,
    u.cod_usuario,
...
LEFT JOIN 
    tb_produto prod ON pp.cod_produto = prod.cod_produto
WHERE 
    ha.cod_hamburguer= cod_hamburguer;
END;
// delimiter ;
