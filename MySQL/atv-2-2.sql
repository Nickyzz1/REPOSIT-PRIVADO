...
delimiter //
create procedure pc_changePrice()
begin
start transaction;
...
-- Adicione um novo cliente com nome = 'Laura', email = 'laura@example.com', e telefone = '(11) 2222-3333'.
--  Se o e-mail já existir na tabela de clientes, execute um ROLLBACK. Caso contrário, xecute um COMMIT.


delimiter //
create procedure pc_adcCliente(nome varchar(100), parametroEmail varchar(100), tel varchar(20))
begin
start transaction;
...
email
from clientes
where email = parametroEmail);

insert into clientes (nome, email, telefone) values (nome, parametroEmail, tel);

if @emailExiste = null then
commit;
else
rollback;
end if;
...
call pc_adcCliente('laura', 'laura@example.com', '(11) 2222-3333');
...
delimiter //
create procedure pc_updateVehicle()
begin
start transaction;
